...
async function handleSignup() {
  ...
  try {
    const res = await fetch("http://localhost:4000/api/v1/signup", { // change if your route is different
      method: "POST",
    ...
    document.getElementById("signup-message").innerText = data.message || "Signup complete!";
  } catch (err) {
...
async function handleSignup() {
  ...
  try {
    const res = await fetch("http://localhost:4000/api/v1/signup", { // ✅ corrected
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, email, password }),
    });
...
async function handleLogin() {
  ...
  try {
    const res = await fetch("http://localhost:4000/api/v1/login", { // ✅ corrected
      method: "POST",
      headers: { "Content-Type": "application/json" },
